...
namespace winrt::RNMaps::implementation
{
        ...
        RNMapIconModule::NativeProps() noexcept
    {
        ...
        // not applicable - nativeProps.Insert(L"centerOffset", ViewManagerPropertyType::Map);
        // not applicable - nativeProps.Insert(L"calloutOffset", ViewManagerPropertyType::Map);
        nativeProps.Insert(L"anchor", ViewManagerPropertyType::Map);
        nativeProps.Insert(L"calloutAnchor", ViewManagerPropertyType::Map);
        // not applicable - nativeProps.Insert(L"flat", ViewManagerPropertyType::Boolean);
        nativeProps.Insert(L"identifier", ViewManagerPropertyType::String);
        // not applicable - nativeProps.Insert(L"rotation", ViewManagerPropertyType::Number);
        nativeProps.Insert(L"draggable", ViewManagerPropertyType::Boolean);
        // not applicable - nativeProps.Insert(L"tracksViewChanges", ViewManagerPropertyType::Boolean);
        // not applicable - /nativeProps.Insert(L"tracksInfoWindowChanges", ViewManagerPropertyType::Boolean);
        // not applicable - nativeProps.Insert(L"stopPropagation", ViewManagerPropertyType::Boolean);
        // not applicable - nativeProps.Insert(L"opacity", ViewManagerPropertyType::Number);
        // not applicable - nativeProps.Insert(L"isPreselected", ViewManagerPropertyType::Boolean);
        // not applicable - nativeProps.Insert(L"key", ViewManagerPropertyType::String);
    ...
    void RNMapIconModule::AddToMap(MapControl const& map, int64_t index)
    {
        if (m_coordinate != BasicGeoposition{})
        {
            UpdateMapIcon();
            map.MapElements().InsertAt(index, m_mapIcon);
        }
    }
    ...
    void RNMapIconModule::UpdateMapIcon()
    {
        ...
        {
            Uri uri{ to_hstring("ms-appx:///" + m_imagePath) };
